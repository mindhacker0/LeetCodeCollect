 ...
 * @return {string}
*/
var serialize = function(root) {//序列化二叉树
    // let stack = [[root,0]];
    // while(stack.length){
    //     let [node,index] = stack.pop();
    //     arr[index] = node.val;
    //     if(node.left) stack.push([node.left,index*2+1]);
    //     if(node.right) stack.push([node.right,index*2+2]);
    // }
    let str="[";
        function tranverse(node,index){
            if(node===null) return str+='null';
            str+=node.val;
            tranverse(node.left,index*2+1);
            tranverse(node.right,index*2+2);
        }
        tranverse(root,0);
    str+="]";
    return str;
